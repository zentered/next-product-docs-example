...
All Vault issuers share common configuration for requesting certificates, namely
the server, path, and CA bundle:
...
> **Warning**: This configuration is incomplete as no authentication methods
> have been added.
...
the secret ID key must be stored within a Kubernetes `Secret` that resides in
the same namespace as the `Issuer`, or otherwise inside the
...
> **Note**: cert-manager does not refresh these token automatically and so
> another process must be put in place to do this.
...
configuration is valid. Replace `issuers` here with `clusterissuers` if that is
